...
const Banners = () => {
    ...
    const [bannersForDelete, setBannersForDelete] = useState<string[]>([]);
    ...
    const handleImageUpload = (index: number, imageURL: string, file: File) => {
                ...
                {[...Array(3)].map((_, index) => (
                    <div key={index} className='w-full lg:w-1/2 xl:w-1/3 p-4'>
                        {typeof banners[index] === 'string' ? (
                            <>
                                <Image width={361} height={133} src={banners[index]} alt='' className='w-full h-auto' />
